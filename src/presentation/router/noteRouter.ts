...
import { authServices, noteServices } from "../../application/services/instance";
...
export const noteRouter = new Elysia({ prefix: "/v1/notes" })
    ...
    .post("/", async ({ body, user }) => {
        ...
        const newNote = await noteServices.create({ title, content, authorId: user?.id as string });
    ...
    .patch("/:id", async ({ body, params }) => {
        ...
        const updatedNote = await noteServices.update(params.id, { title, content });
